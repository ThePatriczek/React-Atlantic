...
export const ButtonList: FC<ButtonListProps> = (props) => {
	...
	const disabledMaxCondition: boolean = isDisabled || currentPage === count || count === 0;
	const disabledMinCondition: boolean = isDisabled || currentPage === 1 || count === 0;
	...
	const noMiddleScenario = (list: number[], right: boolean) => {
		if (count > 6) {
			...
			for (let i: number = right ? count - 4 : 2; i < (right ? count : 1 + 5); i++) {
				list.push(i);
			}
			if (!right && showThreeDots) {
				list.push(-1);
	...
	const generateItem = (item: number): JSX.Element => {
		if (isSimple) {
			return (
		...
		if (isBadge) {
			return <Badge size={'large'} type={item === currentPage ? 'primary' : 'default'} />;
		}
