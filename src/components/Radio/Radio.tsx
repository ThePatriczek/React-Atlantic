...
export const Radio: React.FC<React.PropsWithChildren<RadioProps>> & {
	...
	const { isDefaultChecked, position, isDisabled, className, children, value } = props;
	...
	return (
		...
		>
			{position === 'right' && <StyledRadioSpan isDisabled={isDisabled}>{children}</StyledRadioSpan>}

			<StyledRadioInputShown isDisabled={isDisabled}>
				<StyledRadioMark isDisabled={isDisabled} isChecked={checked}>
					<StyledRadioInputHidden onChange={onChange} checked={checked} disabled={isDisabled} value={value} />
				</StyledRadioMark>
			</StyledRadioInputShown>

			{position === 'left' && <StyledRadioSpan isDisabled={isDisabled}>{children}</StyledRadioSpan>}
		</StyledRadioLabel>
	);
};
