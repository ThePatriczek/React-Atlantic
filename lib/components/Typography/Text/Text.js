...
export var Text = function (props) {
    var children = props.children, type = props.type, isCopyable = props.isCopyable, isEditable = props.isEditable, onEditConfirm = props.onEditConfirm, className = props.className, element = props.element, editText = props.editText, tooltipProps = props.tooltipProps;
    var id = v4();
    var _a = useState(false), isEditing = _a[0], setEditing = _a[1];
    var _b = useState(children), value = _b[0], setValue = _b[1];
    var _c = useState(props.copyText), copyText = _c[0], setCopyText = _c[1];
    var copyBtnRef = createRef();
    var editBtnRef = createRef();
    ...
    if (isEditable && isEditing) {
        ...
        return (React.createElement(TextArea, { onChange: function (val) { return setValue(val); }, value: value, onEnterPress: onEnterPress, onBlur: function () {
                setValue(children);
                setEditing(false);
            }, autoFocus: true, iconRight: 'enter' }));
    }
    var Content = (React.createElement(Fragment, { key: value },
        value,
        isEditable && (React.createElement(StyledTypoButton, { onClick: onEdit, "data-tip": true, "data-for": id, onMouseEnter: function () {
                if (editBtnRef.current) {
            ...
            React.createElement(Tooltip, __assign({ id: id }, tooltipProps), editText))),
        isCopyable && (React.createElement(StyledTypoButton, { onClick: onCopy, "data-tip": true, "data-for": id, ref: copyBtnRef, key: copyText, onMouseEnter: function () {
                if (copyBtnRef.current) {
                    Tooltip.show(copyBtnRef.current);
                }
            }, onMouseLeave: function () {
            ...
            React.createElement(Tooltip, __assign({ id: id }, tooltipProps), copyText)))));
    switch (element) {
        case 'strong':
