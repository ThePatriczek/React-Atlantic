...
export var Input = function (props) {
    var transferFocus = props.transferFocus, defaultValue = props.defaultValue, isDisabled = props.isDisabled, placeholder = props.placeholder, id = props.id, onEnterPress = props.onEnterPress, iconLeft = props.iconLeft, iconRight = props.iconRight, ComponentLeft = props.ComponentLeft, ComponentRight = props.ComponentRight, autoFocus = props.autoFocus, isAlternative = props.isAlternative, size = props.size, isLoading = props.isLoading, className = props.className, autoComplete = props.autoComplete, htmlType = props.htmlType, isFullWidth = props.isFullWidth, handlersWithEvent = props.handlersWithEvent;
    useEffect(function () {
        if (!!iconRight && !!ComponentRight) {
            var err = new Error("Only one of iconRight or componentRight can be defined at the same time");
            throw err;
        }
    }, [iconRight, ComponentRight]);
    useEffect(function () {
        if (!!iconRight && !!ComponentRight) {
            var err = new Error("Only one of iconLeft or componentLeft can be defined at the same time");
            throw err;
        }
    ...
    var AlternativeComponent = (React.createElement(StyledInputWrapperAlt, { size: size, isFocused: transferFocus || isFocused, isLeftComponent: !!iconLeft, isRightComponent: !!(iconRight || isLoading), hasValue: !!val, isDisabled: isDisabled, onClick: function () {
            if (ref.current) {
                ref.current.focus();
            }
        }, className: className, isFullWidth: isFullWidth }, iconLeft || iconRight ? (React.createElement(StyledInputWrapper, { size: size, isFocused: typeof transferFocus !== 'undefined' ? transferFocus : isFocused, isLeftComponent: !!iconLeft, isRightComponent: !!(iconRight || isLoading), isDisabled: isDisabled, onClick: function () {
            if (ref.current) {
                ref.current.focus();
            }
        ...
        !!ComponentRight && !isLoading && ComponentRight)) : (React.createElement(React.Fragment, null,
        Component,
        React.createElement("label", null, placeholder)))));
    if (isAlternative && !isDisabled) {
        return AlternativeComponent;
    }
    return (React.createElement(StyledInputWrapper, { isFocused: isFocused, isLeftComponent: !!(iconLeft || ComponentLeft), isRightComponent: !!(isLoading || iconRight || ComponentRight), isDisabled: isDisabled, size: size, onClick: function () {
            if (ref.current) {
                ref.current.focus();
            }
